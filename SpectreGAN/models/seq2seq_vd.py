...
def gen_encoder(hparams, inputs, sequence_length, targets_present, is_training, reuse=None):
  ...
  with tf.variable_scope('encoder', reuse=reuse):
    ...
    with tf.variable_scope('rnn') as scope:
      ...
      hidden_states, state = tf.nn.dynamic_rnn(
          cell, masked_rnn_inputs, initial_state=state, scope=scope, sequence_length=sequence_length)
      if is_training:
      ...
      _, real_state = tf.nn.dynamic_rnn(
          cell, real_rnn_inputs, initial_state=real_state, scope=scope, sequence_length=sequence_length)
...
def gen_decoder(hparams,
  ...
  with tf.variable_scope('decoder', reuse=reuse):
    ...
    with tf.variable_scope('rnn'):
      ...
      for t in xrange(FLAGS.sequence_length):
        ...
        # Add to lists.
        sequence.append(output) # (400,)
        log_probs.append(log_prob) # (400,)
        logits.append(logit) # (400,384)
      if FLAGS.gen_training_strategy == 'cross_entropy':
  ...
  print((tf.stack(sequence, axis=1), logits, tf.stack(log_probs, axis=1)))
  
  
  return (tf.stack(sequence, axis=1), logits, tf.stack(log_probs, axis=1))


def dis_encoder(hparams, masked_inputs,sequence_length, is_training, reuse=None,
                embedding=None):
  ...
  with tf.variable_scope('encoder', reuse=reuse):
    ...
    with tf.variable_scope('rnn'):
      ...
      rnn_out, state_dis  = tf.nn.dynamic_rnn(
          cell_dis, masked_rnn_inputs, initial_state=state_dis, sequence_length=sequence_length)
      
      rnn_out = tf.transpose(rnn_out, perm=[1,0,2])
      if is_training:
        rnn_out = tf.map_fn((lambda x: x*output_mask), elems=rnn_out, parallel_iterations=10)
      rnn_out = tf.transpose(rnn_out, perm=[1,0,2])

  return (rnn_out, state_dis)
