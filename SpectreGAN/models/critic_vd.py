...
def critic_seq2seq_vd_derivative(hparams, sequence, is_training, sequence_length, reuse=None ):
  ...
  with tf.variable_scope(
      'dis/decoder/rnn', reuse=True) as dis_scope:
  ...
  with tf.variable_scope('critic', reuse=reuse):
    ...
    with tf.variable_scope('rnn') as vs:
      ...
      rnn_out, state_dis = tf.nn.dynamic_rnn(
        cell_critic, rnn_inputs, initial_state=state_dis, scope=dis_scope, sequence_length=sequence_length)
      ...
      values = tf.contrib.layers.linear(rnn_out, 1, scope=vs)
      print(values)
